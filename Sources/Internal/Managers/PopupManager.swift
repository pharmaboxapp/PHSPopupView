...
extension PopupManager {
    ...
    static func dismissPopupAfter(_ popup: any Popup, _ seconds: Double) {
        shared.popupsToBeDismissed[popup.id] = DispatchSource.createAction(deadline: seconds) {
            performOperation(.remove(id: popup.id))
        }
    }
    
    static func hideOverlay(_ popup: any Popup) {
        shared.popupsWithoutOverlay.append(popup.id)
    }
}
...
private extension PopupManager {
    static func removePopupFromStackToBeDismissed(_ operation: StackOperation) {
        switch operation {
        case .removeLast:
            shared.popupsToBeDismissed.removeValue(forKey: shared.views.last?.id ?? "")
        case .remove(let id):
...
private extension [any Popup] {
    func canBeInserted(_ popup: some Popup) -> Bool {
        !contains(where: { $0.id == popup.id })
    }
}
